...
pub = None             # ROS publisher for cmd_vel
linear_limit = 1.0     # Linear speed limit (m/s)
angular_limit = 5.0    # Angular limit (rad/s)
MAX_STEERING_DEG = 45.0  # Max steering angle in degrees
...
def process_remote_frame(image):
    global started, video_writer, video_out_path, start_time, pub
            ...
            for hand_landmarks, handedness in hands_data:
                ...
                if right_angle is not None and left_angle is not None:
                    ...
                    if pub is not None:
                        ...
                        linear_x = (speed / MAX_SPEED) * linear_limit
                        ...
                        # Display ROS control values on screen
                        cv2.putText(image, f"ROS Linear: {linear_x:.2f} m/s", (10, 280), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                        cv2.putText(image, f"ROS Angular: {angular_z:.2f} rad/s", (10, 320), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                        
                        # Log control values
                        rospy.loginfo(f"Linear X: {linear_x:.2f} m/s, Angular Z: {angular_z:.2f} rad/s, Steering: {steering_angle:.1f} deg")
                ...
                if is_hand_open(hand_landmarks):
                    ...
                    if controller == 'pid':
                        reset_pid()
                    # Send stop command when hand opens
                    if pub is not None:
                        pub.publish(Twist())  # All zero twist = stop
    elif pub is not None:
        # No hands or not started - send stop command to robot
        pub.publish(Twist())
...
def cleanup():
    global video_writer, video_out_path, pub
    
    # Send stop command to robot when shutting down
    if pub is not None:
        pub.publish(Twist())
...
def main():
    global pub, linear_limit, angular_limit
    
    # Initialize ROS node
    rospy.init_node('hand_gesture_remote_control', anonymous=True)
    ...
    # Log info
    rospy.loginfo("Hand gesture control ROS node started. Press 'Esc' to quit.")
    rospy.loginfo(f"Using {controller} controller with filter alpha = {filter_alpha}")
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if not process_remote_frame(image):
                break
