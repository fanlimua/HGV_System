...
parser = argparse.ArgumentParser(description='Hand Gesture Control ROS Node with Remote Camera')
parser.add_argument('--server', type=str, required=True,
                    help='Remote camera server IP address')
parser.add_argument('--port', type=int, default=8089,
                    help='Remote camera server port')
...
def connect_to_camera_server(server_ip, server_port):
    # Create client socket
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect((server_ip, server_port))
    print(f"Connected to camera server at {server_ip}:{server_port}")
...
def receive_frame(client_socket, data, payload_size):
...
def main():
    ...
    try:
        client_socket, data, payload_size = connect_to_camera_server(args.server, args.port)
    except ConnectionRefusedError:
        rospy.logerr(f"Could not connect to camera server at {args.server}:{args.port}")
        return
    ...
    try:
        while not rospy.is_shutdown():
            # Receive frame from remote camera
            image, data = receive_frame(client_socket, data, payload_size)
            if image is None:
                rospy.logerr("Lost connection to camera server")
                break
